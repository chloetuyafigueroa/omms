				...
				app.controller('myCtrl1',function($scope,$http,$q,$interval,$timeout,$filter)
				{
					...
					$scope.users= [ { name: "#" },
						...
						var x= [];
						$scope.chatbot={r_id:'',r_type:'',r_name:'',r_address:'',r_contact_number:'',r_particulars:''};
						$scope.initialize=function (){ 
							...
							$scope.Name=$scope.chatbot.r_name;
							$scope.Landmark=$scope.chatbot.r_address;
							$scope.Phone=$scope.chatbot.r_contact_number;
							$scope.Notes=$scope.chatbot.r_particulars;
							$scope.Location="";
							$scope.Gps="0.0","0.0";
							$scope.Lat="0.0";
							$scope.Lon="0.0";
							$scope.ActionTaken="";
							$scope.getType=$scope.chatbot.r_type
							...
							//const result = $scope.test1.find(item => item.category === "town" && $scope.chatbot.r_address.includes(item.value)) || $scope.test1[0];
							//console.log('result:'+JSON.stringify(result));							
							$scope.newTown= $scope.test1.find(item => item.category === "town" && $scope.chatbot.r_address.includes(item.value)) || $scope.test1[0];
							$scope.newBrgys= $scope.test1.find(item => item.category === "barangay" && $scope.chatbot.r_address.includes(item.value.replace(' ',''))) || $scope.test1[1];
			...
			$scope.testRun=function (xd,idx){ 
				...
				//$http.get('someservlet?x='+y0+'&y='+y1+'&z='+y3+'&z1='+y2+'&z2='+y4+'&z3='+y5)
						...
						console.log('table:'+JSON.stringify(response.data));
							var sexy21 = transformJson(JSON.parse(JSON.stringify(response.data)));
			...
			//console.log("Triggered onInputChange function!");
		        // Cancel the previous timeout if the user types again within 3 seconds
		        if (typingTimeout) {
		        ...
		        typingTimeout = $timeout(function() {
		            $scope.triggerFunction(id);  // Call your function here
		        }, 3000); // 3000 ms = 3 seconds
		    };
		
		    // Define the function to be called after 3 seconds of inactivity
		    $scope.triggerFunction = function(id) {
		 	...
		 	function transformJson(json) {
			  return json.map(item => {
			    ...
			    newItem.r_created = moment(item.r_created,"YYYY-MM-DD HH:mm:ss.SSSSSS").format("MM/DD/YY hh:mm:ss A");
			    //newItem.r_selected=true;
			    return newItem;
			  });
			}
			const customOrder = ["r_selected","r_type","r_id","r_name","r_contact_number","r_acct_no","r_address","r_particulars","r_created","r_remarks",];
			...
			$scope.seen=function(id){
				let rowarray = $scope.sexy2.find(row => row.r_id === id);
				let selected=true;
			...
			$scope.selectItem=function(row){ 		
				...
				$http.get('/Joblist/Chatbot',{params:{table:'omms',where:'id='+row.r_id,limit:500}})
						.then(function mySuccs(response){
							...
							$scope.wojo=false;	
							var oid = JSON.parse(JSON.stringify(response.data))[0].oid;
							console.log("OID:"+oid);
							 $http.get('someservlet6?id='+oid)
			...
			$scope.itemChange=function (id){ 
				//console.log('$scope.test1[8]:'+JSON.stringify($scope.test1));
